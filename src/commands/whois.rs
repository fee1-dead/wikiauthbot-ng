...
impl WhoisInfo {
    pub fn create_embed(mut self, discord_user_id: UserId) -> Result<CreateEmbed> {
        ...
        let (fields, blocks): (Vec<_>, Vec<_>) = self.merged.into_iter().filter(|w| w.editcount > 0).map(|wiki| {
            ...
            ((wiki.wiki.clone(), content, inline), wiki.blocked.zip(Some(wiki.wiki)))
        }).unzip();
        ...
        for (block, wiki) in blocks.into_iter().flatten() {
            if !blocked {
                mb.push_line(format!("\n\n<:declined:359850777453264906> **BLOCKED**{}", if self.locked { " ***LOCKED***" } else { "" }));
                blocked = true;
            }
        ...
        let date = chrono::DateTime::parse_from_rfc3339(&self.registration)
            .context("invalid date")?;

        let days: u64 = chrono::offset::Utc::now().signed_duration_since(date).num_days().try_into()?;

        let medal = MEDALS
            .iter()
            .find(|m| m.days < days && m.edits < edits)
            .unwrap_or(MEDALS.last().unwrap()).url;
        ...
        if fields.next().is_some() {
            embed = embed.footer(CreateEmbedFooter::new("Only up to 10 max listed. Click their name at the top to see all info."));
        }
...
pub async fn whois_test(ctx: Context<'_>, name: String) -> Result {
    let crate::Data { client, db: _ } = ctx.data();
    ctx.defer().await?;
    ...
    ctx.send(CreateReply::default().embed(whois.create_embed(ctx.author().id)?)).await?;
...
pub async fn whois(
    ...
    ctx.send(CreateReply::default().ephemeral(true).embed(whois.create_embed(ctx.author().id)?)).await?;
